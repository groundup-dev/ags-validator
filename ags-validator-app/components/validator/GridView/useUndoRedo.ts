...
export function useUndoRedo(
  ...
  const wrappedOnCellsEdited = useCallback(
    (newValues: readonly EditListItem[]) => {
      ...
      console.log("isApplyingUpdate:", isApplyingUpdate); // Log if undo/redo is in progress
      console.log("gridSelectionRef.current:", gridSelectionRef.current); // Log selection state
      ...
      if (!isApplyingUpdate && gridSelectionRef.current) {
        ...
        // Log the new values before batching
        console.log("New values to edit:", newValues);

        newValues.forEach((cell) => {
          const previousValue = getCellContent(
            cell.location
          ) as EditableGridCell;
          ...
          // Log the current batch state after adding an edit
          console.log("Current batch after edit:", currentBatch.current);
        });

        // Log that we're setting the timeout for dispatch
        console.log("Setting timeout for batch dispatch");

        timeout.current = setTimeout(() => {
          if (currentBatch.current) {
            console.log("Dispatching batch:", currentBatch.current);
            dispatch({
              type: "edit",
              batch: currentBatch.current,
            });
      ...
      console.log("Undo history after dispatch:", state.undoHistory);

      // Continue applying the actual edits
      if (onCellsEdited) {
        console.log("Applying edits via onCellsEdited", newValues);
